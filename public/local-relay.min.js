...
function setupFileChannel(connId, dc) {
  ...
  dc.onMessage((raw) => {
    ...
    const chunkNum = (upload.chunkCount || 0) + 1;
    upload.chunkCount = chunkNum;

    console.log(`[FILE DC] Chunk #${chunkNum}: ${chunk.length} bytes, total: ${upload.received + chunk.length}/${upload.size} (${((upload.received + chunk.length)/upload.size*100).toFixed(1)}%)`);
    ...
    // ACK every chunk — client won't advance window until it gets this back.
    // This is the core of reliable flow control: receiver drives the pace.
    if (upload.sendControl) {
      if (!written) {
        // Write buffer full — send backpressure first so client pauses window
        upload.sendControl({
          type: 'file:upload:backpressure',
          connId,
          filename: upload.filename,
          chunkNum,
        });
        // Wait for drain before sending ACK so client only advances after pressure clears
    ...
    // Throttle progress to ~10fps (ACKs already carry received count)
    const now = Date.now();
    if (upload.sendControl && now - (upload.lastProgressAt || 0) >= 100) {
...
async function handleFileUploadStart(connId, msg, sendControl) {
  ...
  for (const c of queued) {
    ...
    // ACK each flushed chunk so client window advances
    sendControl({
      type: 'file:upload:ack',
      connId,
      filename,
