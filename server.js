          ...
          socket.on('docker:command', ({ action, args = [] }) => {
            ...
            if (action === 'list') { cmdSuffix = `ps -a --format "{{json .}}"`; }
              ...
              const stopRmCmd = oldContId
                ...
                : '';
              const siblingCmd = `for c in $(docker ps -aq 2>/dev/null); do cp=$(docker inspect --format "{{index .Config.Labels \\"com.docker.compose.project\\"}}" $c 2>/dev/null); cs=$(docker inspect --format "{{index .Config.Labels \\"com.docker.compose.service\\"}}" $c 2>/dev/null); cn=$(docker inspect --format "{{.Name}}" $c 2>/dev/null | sed "s|^/||"); c_img=$(docker inspect --format "{{.Config.Image}}" $c 2>/dev/null); c_mounts=$(docker inspect --format "{{range .Mounts}}--mount type={{.Type}},source={{if eq .Type \\"volume\\"}}{{.Name}}{{else}}{{.Source}}{{end}},target={{.Destination}} {{end}}" $c 2>/dev/null); svc_target="\${cs:-\$cn}"; [ -z "$svc_target" ] || [ -z "$c_img" ] && continue; is_db=false; echo "$cn $c_img $svc_target" | grep -qiE "mongo|redis|postgres|mysql|mariadb|memcached" && is_db=true; svc_replicas=1; [ "$is_db" = "false" ] && svc_replicas=2; is_match=false; [ -n "${composeProj}" ] && [ "$cp" = "${composeProj}" ] && [ "$cn" != "${oldContId}" ] && [ "$cn" != "${svcName}" ] && is_match=true; [ "$is_db" = "true" ] && [ "$cn" != "${oldContId}" ] && [ "$cn" != "${svcName}" ] && is_match=true; if [ "$is_match" = "true" ]; then if ! docker service inspect "$svc_target" >/dev/null 2>&1 && ! docker service inspect "$cn" >/dev/null 2>&1; then echo "Converting sibling $cn into Swarm service $svc_target (replicas=$svc_replicas)..."; net_aliases="$svc_target"; [ -n "$cn" ] && [ "$cn" != "$svc_target" ] && net_aliases="$net_aliases,$cn"; [ -n "$cs" ] && [ "$cs" != "$svc_target" ] && net_aliases="$net_aliases,$cs"; echo "$cn $svc_target" | grep -qi "mongo" && net_aliases="$net_aliases,mongo,mongodb"; echo "$cn $svc_target" | grep -qi "redis" && net_aliases="$net_aliases,redis"; echo "$cn $svc_target" | grep -qi "postgres" && net_aliases="$net_aliases,postgres,postgresql"; echo "$cn $svc_target" | grep -qiE "mysql|mariadb" && net_aliases="$net_aliases,mysql,mariadb"; net_flag=""; for alias in $(echo "$net_aliases" | tr , ' '); do net_flag="$net_flag --network name=$target_net,alias=$alias"; break; done; net_flag="--network name=$target_net,$(echo $net_aliases | sed 's/,/,alias=/g; s/^/alias=/')"; extra_nets=""; for net in $(docker inspect --format "{{range $k, $v := .NetworkSettings.Networks}}{{$k}} {{end}}" $c 2>/dev/null); do if [ "$net" != "bridge" ] && [ "$net" != "host" ] && [ "$net" != "none" ] && [ "$net" != "$target_net" ]; then driver=$(docker network inspect "$net" --format "{{.Driver}}" 2>/dev/null); if [ "$driver" = "overlay" ]; then extra_nets="$extra_nets --network $net"; fi; fi; done; env_file=$(mktemp); docker inspect --format "{{range .Config.Env}}{{println .}}{{end}}" $c 2>/dev/null > $env_file; docker stop $c 2>/dev/null || true; docker rm $c 2>/dev/null || true; docker service create --name "$svc_target" --replicas $svc_replicas $net_flag $extra_nets $c_mounts --env-file $env_file "$c_img" 2>&1; rm -f $env_file; fi; fi; done; `;
               ...
               cmdSuffix = `image ls -a --format "{{json .}}"`;
                ...
                cmdSuffix = `sh -c 'docker swarm init ${advertiseFlag} 2>&1; STATUS=$?; if [ $STATUS -eq 0 ]; then docker swarm update --task-history-limit 1 2>/dev/null || true; fi; exit 0'`;
                 ...
                 if (rawMounts) {
                  ...
                  const siblingCmd = `for c in $(docker ps -aq 2>/dev/null); do cp=$(docker inspect --format "{{index .Config.Labels \\"com.docker.compose.project\\"}}" $c 2>/dev/null); cs=$(docker inspect --format "{{index .Config.Labels \\"com.docker.compose.service\\"}}" $c 2>/dev/null); cn=$(docker inspect --format "{{.Name}}" $c 2>/dev/null | sed "s|^/||"); c_img=$(docker inspect --format "{{.Config.Image}}" $c 2>/dev/null); c_mounts=$(docker inspect --format "{{range .Mounts}}--mount type={{.Type}},source={{if eq .Type \\"volume\\"}}{{.Name}}{{else}}{{.Source}}{{end}},target={{.Destination}} {{end}}" $c 2>/dev/null); svc_target="\${cs:-\$cn}"; [ -z "$svc_target" ] || [ -z "$c_img" ] && continue; is_db=false; echo "$cn $c_img $svc_target" | grep -qiE "mongo|redis|postgres|mysql|mariadb|memcached" && is_db=true; svc_replicas=1; [ "$is_db" = "false" ] && svc_replicas=2; is_match=false; [ -n "${composeProj}" ] && [ "$cp" = "${composeProj}" ] && [ "$cn" != "${oldContId}" ] && [ "$cn" != "${svcName}" ] && is_match=true; [ "$is_db" = "true" ] && [ "$cn" != "${oldContId}" ] && [ "$cn" != "${svcName}" ] && is_match=true; if [ "$is_match" = "true" ]; then if ! docker service inspect "$svc_target" >/dev/null 2>&1 && ! docker service inspect "$cn" >/dev/null 2>&1; then echo "Converting sibling $cn into Swarm service $svc_target (replicas=$svc_replicas)..."; net_aliases="$svc_target"; [ -n "$cn" ] && [ "$cn" != "$svc_target" ] && net_aliases="$net_aliases,$cn"; [ -n "$cs" ] && [ "$cs" != "$svc_target" ] && net_aliases="$net_aliases,$cs"; echo "$cn $svc_target" | grep -qi "mongo" && net_aliases="$net_aliases,mongo,mongodb"; echo "$cn $svc_target" | grep -qi "redis" && net_aliases="$net_aliases,redis"; echo "$cn $svc_target" | grep -qi "postgres" && net_aliases="$net_aliases,postgres,postgresql"; echo "$cn $svc_target" | grep -qiE "mysql|mariadb" && net_aliases="$net_aliases,mysql,mariadb"; net_flag="--network name=$target_net,$(echo $net_aliases | sed 's/,/,alias=/g; s/^/alias=/')"; extra_nets=""; for net in $(docker inspect --format "{{range $k, $v := .NetworkSettings.Networks}}{{$k}} {{end}}" $c 2>/dev/null); do if [ "$net" != "bridge" ] && [ "$net" != "host" ] && [ "$net" != "none" ] && [ "$net" != "$target_net" ]; then driver=$(docker network inspect "$net" --format "{{.Driver}}" 2>/dev/null); if [ "$driver" = "overlay" ]; then extra_nets="$extra_nets --network $net"; fi; fi; done; env_file=$(mktemp); docker inspect --format "{{range .Config.Env}}{{println .}}{{end}}" $c 2>/dev/null > $env_file; docker stop $c 2>/dev/null || true; docker rm $c 2>/dev/null || true; docker service create --name "$svc_target" --replicas $svc_replicas $net_flag $extra_nets $c_mounts --env-file $env_file "$c_img" 2>&1; rm -f $env_file; fi; fi; done; `;
