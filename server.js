          ...
          socket.on('docker:command', ({ action, args = [] }) => {
            ...
            if (action === 'list') { cmdSuffix = `ps -a --format "{{json .}}"`; }
              ...
              const stopRmCmd = oldContId
                ...
                : '';
              const siblingCmd = `for c in $(docker ps -aq); do cp=$(docker inspect --format '{{index .Config.Labels "com.docker.compose.project"}}' $c 2>/dev/null); cs=$(docker inspect --format '{{index .Config.Labels "com.docker.compose.service"}}' $c 2>/dev/null); cn=$(docker inspect --format "{{.Name}}" $c 2>/dev/null | sed 's/^\\///'); c_img=$(docker inspect --format "{{.Config.Image}}" $c 2>/dev/null); c_mounts=$(docker inspect --format '{{range .Mounts}}--mount type={{.Type}},source={{.Source}},target={{.Destination}} {{end}}' $c 2>/dev/null); svc_target="\${cs:-\$cn}"; is_db=false; echo "$cn $c_img" | grep -qiE "mongo|redis|postgres|mysql|mariadb|memcached" && is_db=true; db_replicas=1; svc_replicas=1; if [ "$is_db" = "true" ]; then svc_replicas=1; fi; if [ -n "${composeProj}" ] && [ "$cp" = "${composeProj}" ] && [ "$cn" != "${oldContId}" ] && [ "$cn" != "${svcName}" ] && [ -n "$svc_target" ] && [ -n "$c_img" ]; then if ! docker service inspect "$svc_target" >/dev/null 2>&1 && ! docker service inspect "$cn" >/dev/null 2>&1; then echo "Converting sibling $cn into Swarm service $svc_target (replicas=$svc_replicas)..."; docker stop $c 2>/dev/null || true; docker rm $c 2>/dev/null || true; docker service create --name "$svc_target" --replicas $svc_replicas --network "$target_net" $c_mounts "$c_img" 2>/dev/null || true; fi; elif echo "$cn $c_img" | grep -qiE "mongo|redis|postgres|mysql|mariadb|memcached" && [ "$cn" != "${oldContId}" ] && [ "$cn" != "${svcName}" ]; then if ! docker service inspect "$svc_target" >/dev/null 2>&1 && ! docker service inspect "$cn" >/dev/null 2>&1; then echo "Auto-converting database $cn into Swarm service $svc_target (replicas=1)..."; docker stop $c 2>/dev/null || true; docker rm $c 2>/dev/null || true; docker service create --name "$svc_target" --replicas 1 --network "$target_net" $c_mounts "$c_img" 2>/dev/null || true; fi; fi; done; `;
              ...
              cmdSuffix = `rm -f /tmp/deploy_${safeName}.log; touch /tmp/deploy_${safeName}.log; nohup sh -c '${cmd} 2>&1 | tee /tmp/deploy_${safeName}.log; echo "---FINISHED---" >> /tmp/deploy_${safeName}.log' >/dev/null 2>&1 & echo STARTED`;
            } else if (action === 'swarm:build-deploy:status' && args.length >= 1) {
              const serviceName = String(args[0] || '').replace(/[^a-zA-Z0-9._-]/g, '');
              const safeName = serviceName.replace(/[^a-z0-9]/gi, '_');
              cmdSuffix = `(if [ -f "/tmp/deploy_${safeName}.log" ]; then RUNNING=$(ps aux 2>/dev/null | grep -v grep | grep "docker.*${serviceName}" | wc -l); if [ "$RUNNING" = "0" ] && ! grep -q "---FINISHED---" "/tmp/deploy_${safeName}.log"; then echo "---FINISHED---" >> /tmp/deploy_${safeName}.log; fi; tr '\\r' '\\n' < "/tmp/deploy_${safeName}.log" | tail -n 25; else echo "INITIALIZING..."; fi); exit 0`;
            } else if (action === 'swarm:get-workdir' && args.length >= 1) {
              const serviceName = String(args[0] || '').replace(/[^a-zA-Z0-9._-]/g, '');
              if (!serviceName) return socket.emit('docker:error', 'Invalid Service Name');
              cmdSuffix = `sh -c 'sName="${serviceName}"; svc_dir=$(docker service inspect $sName --format "{{index .Spec.Labels \\"com.docker.compose.project.working_dir\\"}}" 2>/dev/null); [ -z "$svc_dir" ] && svc_dir=$(docker service inspect $sName --format "{{index .Spec.Labels \\"project.directory\\"}}" 2>/dev/null); if [ -z "$svc_dir" ]; then for cid in $(docker ps -aq 2>/dev/null); do c_proj=$(docker inspect --format "{{index .Config.Labels \\"com.docker.compose.project\\"}}" $cid 2>/dev/null); c_name=$(docker inspect --format "{{.Name}}" $cid 2>/dev/null | sed "s/^\\///"); if [ "$c_name" = "$sName" ] || [ "$c_proj" = "$sName" ] || [ "$c_name" = "\${sName}-1" ]; then svc_dir=$(docker inspect --format "{{index .Config.Labels \\"com.docker.compose.project.working_dir\\"}}" $cid 2>/dev/null); [ -n "$svc_dir" ] && break; fi; done; fi; if [ -z "$svc_dir" ] || [ ! -d "$svc_dir" ]; then for p in "test/$sName" "/root/test/$sName" "$HOME/test/$sName" "/root/$sName" "$HOME/$sName" "/var/www/$sName" "/opt/$sName" "./test/$sName"; do if [ -d "$p" ]; then svc_dir=$(cd "$p" 2>/dev/null && pwd); [ -n "$svc_dir" ] && break; fi; done; fi; if [ -z "$svc_dir" ]; then found=$(find /root /home /var/www /opt . -maxdepth 4 -name "$sName" -type d 2>/dev/null | head -1); [ -n "$found" ] && svc_dir=$(cd "$found" 2>/dev/null && pwd); fi; echo "WORKDIR:\${svc_dir:-$(pwd)}"'`;
               ...
               cmdSuffix = `image ls -a --format "{{json .}}"`;
                ...
                cmdSuffix = `sh -c 'docker swarm init ${advertiseFlag} 2>&1; STATUS=$?; if [ $STATUS -eq 0 ]; then docker swarm update --task-history-limit 1 2>/dev/null || true; fi; exit 0'`;
                 ...
                 if (rawMounts) {
                  ...
                  const siblingCmd = `for c in $(docker ps -aq); do cp=$(docker inspect --format '{{index .Config.Labels "com.docker.compose.project"}}' $c 2>/dev/null); cs=$(docker inspect --format '{{index .Config.Labels "com.docker.compose.service"}}' $c 2>/dev/null); cn=$(docker inspect --format "{{.Name}}" $c 2>/dev/null | sed 's/^\\///'); c_img=$(docker inspect --format "{{.Config.Image}}" $c 2>/dev/null); c_mounts=$(docker inspect --format '{{range .Mounts}}--mount type={{.Type}},source={{.Source}},target={{.Destination}} {{end}}' $c 2>/dev/null); svc_target="\${cs:-\$cn}"; is_db=false; echo "$cn $c_img" | grep -qiE "mongo|redis|postgres|mysql|mariadb|memcached" && is_db=true; svc_replicas=1; if [ "$is_db" = "true" ]; then svc_replicas=1; fi; if [ -n "${composeProj}" ] && [ "$cp" = "${composeProj}" ] && [ "$cn" != "${oldContId}" ] && [ "$cn" != "${svcName}" ] && [ -n "$svc_target" ] && [ -n "$c_img" ]; then if ! docker service inspect "$svc_target" >/dev/null 2>&1 && ! docker service inspect "$cn" >/dev/null 2>&1; then echo "Converting sibling $cn into Swarm service $svc_target (replicas=$svc_replicas)..."; docker stop $c 2>/dev/null || true; docker rm $c 2>/dev/null || true; docker service create --name "$svc_target" --replicas $svc_replicas --network "$target_net" $c_mounts "$c_img" 2>/dev/null || true; fi; elif echo "$cn $c_img" | grep -qiE "mongo|redis|postgres|mysql|mariadb|memcached" && [ "$cn" != "${oldContId}" ] && [ "$cn" != "${svcName}" ]; then if ! docker service inspect "$svc_target" >/dev/null 2>&1 && ! docker service inspect "$cn" >/dev/null 2>&1; then echo "Auto-converting database $cn into Swarm service $svc_target (replicas=1)..."; docker stop $c 2>/dev/null || true; docker rm $c 2>/dev/null || true; docker service create --name "$svc_target" --replicas 1 --network "$target_net" $c_mounts "$c_img" 2>/dev/null || true; fi; fi; done; `;
