...
`;
  ...
  res = res.replace(/(src|href|action)=([\"'])\/((?!\/)[^\"']*)([\"'])/gi,
    (m, attr, q, path, q2) => {
      ...
      const full = '/' + path;
      return `${attr}=${q}${pathProxyBase}${encodeProxyPath(full)}${q2}`;
    });
  ...
  res = res.replace(/url\((["']?)\/((?!\/)[^)]*?)(["']?)\)/gi,
    (m, q, path, q2) => {
      if (path === 'cdn-cgi' || path.startsWith('cdn-cgi/')) return m;
      const full = '/' + path;
      return `url(${q}${pathProxyBase}${encodeProxyPath(full)}${q2})`;
    });
...
function rewriteRootAssetRefs(text, connectionId, port) {
  ...
  // verbatim (hashed or not — images/fonts too). No cache-epoch query is added:
  // Hermes' Rolldown runtime builds lazy-chunk URLs at LOAD TIME from numeric
  // chunk ids (no version-query suffix anywhere in its bundle), so a suffix on the
  // modulepreload links would make the browser treat the preloaded and the
  // lazily-imported copies as two different modules — a context split that
  // crashes the app back to the green splash (measured 2026-09-13:
  // "useSystemActions must be used within a SystemActionsProvider"). The
