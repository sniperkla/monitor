...
import {
  ...
  HardDrive, Download, Upload, ArrowLeftRight, History,
  CircleCheckBig, Zap, Server
} from 'lucide-react';

const STORAGE_KEY = 'server-backup-onboarding-completed';
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 40 }}>
            ...
            <h1 style={{
              ...
              {t(`serverBackup.onboarding.steps.${meta.id}.title`)}
            </h1>
            <p style={{ margin: '0 auto', maxWidth: 520, fontSize: 16, color: '#94a3b8', lineHeight: 1.6 }}>
              <TypedText text={t(`serverBackup.onboarding.steps.${meta.id}.description`)} speed={15} delay={300} />
            </p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 40 }}>
            {[
              { icon: Download, title: 'Backup', desc: 'Dump web apps, databases, Docker, and system files' },
              { icon: Upload, title: 'Restore', desc: 'One-click restore from your backup history' },
              { icon: ArrowLeftRight, title: 'Transfer', desc: 'Migrate backups between servers' },
              { icon: History, title: 'Jobs', desc: 'Schedule and automate recurring backups' },
            ].map((feature, i) => (
              <div key={i} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
            ...
            >Start Tour <ChevronRight size={18} /></button>
          </div>
        </div>
      </div>
    );
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{ margin: '0 0 32px', fontSize: 15, color: '#94a3b8', lineHeight: 1.6 }}>
            {t(`serverBackup.onboarding.steps.${meta.id}.description`)}
          </p>
          <button onClick={onNext} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <span style={{
                  ...
                  {t('serverBackup.onboarding.stepOf', { step: contentStep, total: contentTotal })}
                </span>
              </div>
              ...
              <p style={{ margin: 0, fontSize: 14, color: '#94a3b8', lineHeight: 1.6 }}>
                <TypedText text={t(`serverBackup.onboarding.steps.${meta.id}.description`)} speed={10} delay={100} />
              </p>
              {meta.hasTip && (
                <div style={{
                  ...
                  <span style={{ fontSize: 13, color: '#94a3b8', lineHeight: 1.5 }}>
                    {t(`serverBackup.onboarding.steps.${meta.id}.tip`)}
                  </span>
                </div>
              )}
            </div>
...
export default function ServerBackupOnboarding({ onComplete }) {
...
export function hasCompletedServerBackupOnboarding() {
  if (typeof window === 'undefined') return false;
  return localStorage.getItem(STORAGE_KEY) === 'true';
}

export function resetServerBackupOnboarding() {
