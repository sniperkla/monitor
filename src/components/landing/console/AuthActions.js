...
/* ── GoogleButton — the one white brand button, shared everywhere ── */
...
/* ── Auth actions — the hero console's full option stack ── */
...
/* ── CloserActions — the end-of-story CTA. One primary action; the rest are
      quiet links, so the closer doesn't repeat the hero's button stack. ── */
function CloserActions({
  ...
  return (
    <div>
      <GoogleButton />
      {passkeyError && (
        <p className="text-[10px] text-red-400/90 text-center mt-2.5">{passkeyError}</p>
      )}
      <div className="mt-3.5 flex items-center justify-center gap-2.5 flex-wrap font-mono text-[10px] text-slate-500">
        {passkeySupported && (
          <>
            <button
              type="button"
              onClick={onPasskey}
              disabled={passkeyLoading}
              className="flex items-center gap-1 hover:text-slate-300 transition-colors cursor-pointer disabled:opacity-60"
            >
              {passkeyLoading ? (
