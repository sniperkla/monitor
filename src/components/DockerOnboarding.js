...
import {
  ...
  Box, Layers, Image, HardDrive, Network,
  Play, CircleCheckBig, CircleHelp, Zap, Activity
} from 'lucide-react';

const STORAGE_KEY = 'docker-onboarding-completed';
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 40 }}>
            ...
            <h1 style={{
              ...
              {t(`docker.onboarding.steps.${meta.id}.title`)}
            </h1>
            <p style={{ margin: '0 auto', maxWidth: 520, fontSize: 16, color: '#94a3b8', lineHeight: 1.6 }}>
              <TypedText text={t(`docker.onboarding.steps.${meta.id}.description`)} speed={15} delay={300} />
            </p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 40 }}>
            {[
              { icon: Box, title: 'Containers', desc: 'Start, stop, inspect running containers' },
              { icon: Layers, title: 'Swarm Services', desc: 'Manage Docker Swarm deployments' },
              { icon: Image, title: 'Images', desc: 'Pull, tag, and delete Docker images' },
              { icon: HardDrive, title: 'Volumes & Networks', desc: 'Manage persistent storage and networking' },
            ].map((feature, i) => (
              <div key={i} style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{ margin: '0 0 32px', fontSize: 15, color: '#94a3b8', lineHeight: 1.6 }}>
            {t(`docker.onboarding.steps.${meta.id}.description`)}
          </p>
          <button onClick={onNext} style={{
            display: 'inline-flex', alignItems: 'center', gap: 8,
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <span style={{
                  ...
                  {t('docker.onboarding.stepOf', { step: contentStep, total: contentTotal })}
                </span>
              </div>
              ...
              <p style={{ margin: 0, fontSize: 14, color: '#94a3b8', lineHeight: 1.6 }}>
                <TypedText text={t(`docker.onboarding.steps.${meta.id}.description`)} speed={10} delay={100} />
              </p>
              {meta.hasTip && (
                <div style={{
                  ...
                  <span style={{ fontSize: 13, color: '#94a3b8', lineHeight: 1.5 }}>
                    {t(`docker.onboarding.steps.${meta.id}.tip`)}
                  </span>
                </div>
              )}
            </div>
...
export default function DockerOnboarding({ onComplete }) {
  const { t } = useTranslation();
...
export function hasCompletedDockerOnboarding() {
  if (typeof window === 'undefined') return false;
  return localStorage.getItem(STORAGE_KEY) === 'true';
}

export function resetDockerOnboarding() {
