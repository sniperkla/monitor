...
const STORAGE_KEY = 'virus-onboarding-completed';
...
const CONTENT = {
  welcome: { title: 'Welcome to Virus Scanner', description: 'Your command center for hunting cryptominers, webshells and intrusions — powered by trusted engines like ClamAV, LMD and Wazuh.' },
  serverSelect: { title: 'Pick Your Server', description: 'Choose which SSH connection to audit. Every check runs over your existing encrypted connection.' },
  runModes: { title: 'Three Scan Modes', description: 'Hover ⚡ Quick / ▶ Deep / 💽 Full to see exactly what each mode checks for. Quick never launches heavy scans — Deep and Full do.' },
  statusBadges: { title: 'Live Engine Status', description: 'These badges show realtime state of every engine: Idle, Scanning, Done or Stopped — updated every few seconds.' },
  engines: { title: 'Trusted Detection Engines', description: 'Install ClamAV, Maldet (LMD) or Wazuh with one click. Installs run detached at low CPU priority so production work is never affected.' },
  findings: { title: 'Review Your Findings', description: 'Every detection lands in the Findings tab, tagged by severity and source — quarantine, delete or ignore with one click.' },
  history: { title: 'Scan History', description: 'Every past scan is archived here — timestamps, mode, engine results and findings counts, so you can prove the server was clean at any point in time.' },
  done: { title: "You're Ready!", description: 'Run a Quick Scan anytime — finished antivirus results are collected automatically.' },
};
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 40 }}>
            ...
            <h1 style={{
              ...
              {(CONTENT[meta.id] || {}).title}
            </h1>
            <p style={{ margin: '0 auto', maxWidth: 520, fontSize: 16, color: '#94a3b8', lineHeight: 1.6 }}>
              <TypedText text={(CONTENT[meta.id] || {}).description} speed={15} delay={300} />
            </p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 40 }}>
            {[
              { icon: Bug, title: 'Trusted Antivirus', desc: 'ClamAV + LMD detections flow straight into your Findings tab' },
            { icon: HardDrive, title: 'Three Scan Modes', desc: '⚡ Quick health checks · ▶ Deep common paths · 💽 Full entire disk' },
            { icon: Radar, title: 'Wazuh HIDS Alerts', desc: 'Level-7+ intrusion alerts surface automatically in your findings' },
            { icon: FileWarning, title: 'Actionable Findings', desc: 'Quarantine, delete or kill threats directly from the results list' },].map((feature, i) => (
              <div key={i} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
            ...
            >{'Start the tour'} <ChevronRight size={18} /></button>
          </div>
        </div>
      </div>
    );
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button onClick={onNext} style={{
            ...
            onMouseLeave={e => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.boxShadow = `0 8px 32px ${meta.color}40`; }}
          >
            <Sparkles size={18} /> {'Start scanning'}
          </button>
        </div>
      </div>
    );
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <span style={{
                  ...
                  {`Step ${contentStep} of ${contentTotal}`}
                </span>
              </div>
              ...
              <p style={{ margin: 0, fontSize: 14, color: '#94a3b8', lineHeight: 1.6 }}>
                <TypedText text={(CONTENT[meta.id] || {}).description} speed={10} delay={100} />
              </p>
              {meta.hasTip && (
                <div style={{
                  ...
                  <span style={{ fontSize: 13, color: '#94a3b8', lineHeight: 1.5 }}>
                    {(CONTENT[meta.id] || {}).tip || "Follow the highlighted element"}
                  </span>
                </div>
              )}
            </div>
...
export default function VirusScanOnboarding({ onComplete }) {
...
export function hasCompletedVirusScanOnboarding() {
  if (typeof window === 'undefined') return false;
  return localStorage.getItem(STORAGE_KEY) === 'true';
}

export function resetVirusScanOnboarding() {
