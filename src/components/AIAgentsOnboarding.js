...
// Copy lives in i18n under `ai.onboarding.*` (en + th). One self-contained
...
import {
  ...
  BrainCircuit, Server, LayoutGrid, Cable,
  CircleCheckBig, Zap
} from 'lucide-react';
import { useOnboardingLayout } from '@/hooks/useOnboardingLayout';

const STORAGE_KEY = 'ai-agents-onboarding-completed';
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 40 }}>
            ...
            <h1 style={{
              ...
              {t(`ai.onboarding.steps.${meta.id}.title`)}
            </h1>
            <p style={{ margin: '0 auto', maxWidth: 520, fontSize: 16, color: '#94a3b8', lineHeight: 1.6 }}>
              <TypedText text={t(`ai.onboarding.steps.${meta.id}.description`)} speed={15} delay={300} />
            </p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(200px, 100%), 1fr))', gap: 16, marginBottom: 40 }}>
            {[
              { icon: BrainCircuit, title: t('ai.onboarding.features.agents.title'), desc: t('ai.onboarding.features.agents.desc') },
              { icon: Server, title: t('ai.onboarding.features.server.title'), desc: t('ai.onboarding.features.server.desc') },
              { icon: LayoutGrid, title: t('ai.onboarding.features.tabs.title'), desc: t('ai.onboarding.features.tabs.desc') },
              { icon: Cable, title: t('ai.onboarding.features.relay.title'), desc: t('ai.onboarding.features.relay.desc') },
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{ margin: '0 0 32px', fontSize: 15, color: '#94a3b8', lineHeight: 1.6 }}>
            {t(`ai.onboarding.steps.${meta.id}.description`)}
          </p>
          <button onClick={onNext} style={{
            display: 'inline-flex', alignItems: 'center', gap: 8,
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <span style={{
                  ...
                  {t('ai.onboarding.stepOf', { step: contentStep, total: contentTotal })}
                </span>
              </div>
              ...
              <p style={{ margin: 0, fontSize: 14, color: '#94a3b8', lineHeight: 1.6 }}>
                <TypedText text={t(`ai.onboarding.steps.${meta.id}.description`)} speed={10} delay={100} />
              </p>
              {meta.hasTip && (
                <div style={{
                  ...
                  <span style={{ fontSize: 13, color: '#94a3b8', lineHeight: 1.5 }}>
                    {t(`ai.onboarding.steps.${meta.id}.tip`)}
                  </span>
                </div>
              )}
            </div>
...
export default function AIAgentsOnboarding({ onComplete }) {
  const { t } = useTranslation();
...
export function hasCompletedAIAgentsOnboarding() {
  if (typeof window === 'undefined') return false;
  return localStorage.getItem(STORAGE_KEY) === 'true';
}

export function resetAIAgentsOnboarding() {
