...
export default function AgentSetupWizard({
  ...
  const uninstallCommand = `systemctl --user stop server-monitor-agent.service ssh-monitor-relay.service 2>/dev/null; systemctl --user disable server-monitor-agent.service ssh-monitor-relay.service 2>/dev/null; sudo systemctl stop server-monitor-agent.service ssh-monitor-relay.service 2>/dev/null; tmux kill-session -t monitor-agent 2>/dev/null; tmux kill-session -t monitor-relay 2>/dev/null; pkill -9 -f '[a]gent\\.js' 2>/dev/null; pkill -9 -f '[m]onitor-agent' 2>/dev/null; pkill -9 -f '[l]ocal-relay' 2>/dev/null; rm -rf ~/.config/server-monitor-agent ~/.monitor-agent* /tmp/.agent* /tmp/monitor-agent* 2>/dev/null; echo "✅ Agent purged"`;
