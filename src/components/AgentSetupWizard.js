...
export default function AgentSetupWizard({ 
  ...
  const tmuxCommand = `curl -sSL -H 'Cache-Control: no-cache' '${serverUrl}/monitor-agent.min.js' -o /tmp/.agent.js && NODE_BIN=$(command -v node || which node || echo node) && tmux new-session -d -s monitor-agent "bash -c 'export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:\\$PATH; $NODE_BIN /tmp/.agent.js --server \\"${serverUrl}\\" --token \\"${effectiveToken}\\"${connection?._id ? ` --connection-id \\"${connection._id}\\"` : ''} >> ~/.monitor-agent.log 2>&1'" && rm -f /tmp/.agent.js`;
