...
import {
  ...
  CircleCheck, AlertCircle, LoaderCircle, FolderOpen, Server, Database,
  Shield, FileBox, ChevronRight, RefreshCw, X, CircleHelp
} from 'lucide-react';
              ...
              <ConfigSection title="Docker Backup Options">
                <div className="space-y-2" ref={containerDropdownRef}>
                  ...
                  <div className="relative">
                    ...
                    />
                    {loadingContainers && <LoaderCircle size={12} className="absolute right-3 top-1/2 -translate-y-1/2 animate-spin text-[var(--text-muted)]" />}
                    {/* Dropdown */}
                    {showContainerDropdown && availableContainers.length > 0 && (
                      <div className="absolute z-50 mt-1 w-full max-h-48 overflow-y-auto rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-color)] shadow-xl">
                        ...
                        >
                          <CircleCheck size={12} className={config.containers.length === 0 ? 'opacity-100' : 'opacity-0'} />
                            ...
                            return (
                              ...
                              >
                                <div className={`w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0 ${selected ? 'bg-indigo-500 border-indigo-500' : 'border-[var(--border-color)]'}`}>
                                  {selected && <CircleCheck size={10} className="text-white" />}
                                </div>
              ...
              {outFilePath && !isRunning && (
                <>
                  <button onClick={handleDownload} disabled={isUploadingR2} className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white text-xs font-bold transition-all flex items-center gap-2">
                    {isUploadingR2 ? <LoaderCircle size={14} className="animate-spin" /> : <Download size={14} />}
        ...
        {activeTab === 'restore' && (
          <>
            ...
            <button onClick={handleRestore} disabled={isRestoring || !restoreFile || !restoreTargetId} className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold transition-all disabled:opacity-50 flex items-center gap-2">
              {isRestoring ? <LoaderCircle size={14} className="animate-spin" /> : <Upload size={14} />}
          ...
          <>
            ...
            <div className="flex gap-2">
              <button onClick={handleTransfer} disabled={isTransferring || (!outFilePath && !transferSourceEntry) || !transferTargetId} className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold transition-all disabled:opacity-50 flex items-center gap-2">
                {isTransferring ? <LoaderCircle size={14} className="animate-spin" /> : <ArrowLeftRight size={14} />}
          ...
          <div className="w-[480px] max-h-[80vh] rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border-color)] shadow-2xl flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-2 min-h-[200px]">
              {folderBrowser.loading ? (
                <div className="flex items-center justify-center py-8 text-[var(--text-muted)] text-xs"><LoaderCircle size={14} className="animate-spin mr-2" /> Loading...</div>
          ...
          <div className="max-h-48 overflow-y-auto p-1">
            {filtered.length === 0 ? (
              ...
              filtered.map(o => (
                ...
                >
                  {value === o.value && <CircleCheck size={12} className="text-indigo-400 shrink-0" />}
...
function StatusBadge({ status }) {
  ...
  return (
    <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold border ${cls}`}>
      {status === 'completed' ? <CircleCheck size={11} /> : <AlertCircle size={11} />}
...
function ServerFilesList({ connectionId, apiFetch }) {
  ...
  if (loading) return <div className="text-xs text-[var(--text-muted)]"><LoaderCircle size={12} className="animate-spin inline mr-1" /> Loading...</div>;
