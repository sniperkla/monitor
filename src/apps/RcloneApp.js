...
export default function RcloneApp() {
  const { connections, vaultStatus } = useVault();
  ...
  // Select first connection on mount
  useEffect(() => {
    if (connections && connections.length > 0 && !selectedConnId) {
      setSelectedConnId(connections[0].id || connections[0]._id);
    }
  }, [connections, selectedConnId]);

  // Load Rclone Status when selected connection changes
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg-primary)] text-[var(--text-primary)] font-sans">
      ...
      <div className="px-6 py-4 border-b border-[var(--border-color)] bg-[var(--bg-secondary)] space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-bold flex items-center gap-2">
                Rclone Cloud Backup & Sync
              </h1>
              <p className="text-xs text-[var(--text-muted)]">
                Initialize rclone and manage multi-cloud backups directly from your browser
              </p>
            </div>
          </div>
            ...
            {connections?.map((c) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-3 px-3.5 py-2 rounded-xl border text-left transition-all shrink-0 ${
                    isSelected
                ...
                >
                  <Server size={16} className={isSelected ? 'text-indigo-400' : 'text-[var(--text-muted)]'} />
                  <div>
                    <div className="text-xs font-bold leading-none mb-1">{c.name}</div>
                    <div className="text-[10px] font-mono opacity-80 leading-none">{c.username || 'root'}@{c.host}</div>
                  </div>
                  {isSelected && <CheckCircle2 size={14} className="text-indigo-400 ml-1" />}
                </button>
              );
            })}
          </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[var(--text-muted)]">
                      {rcloneStatus?.version || 'Initialize rclone on this SSH server with 1 click below'}
                    </p>
                  </div>
                </div>
                ...
                {!rcloneStatus?.installed && (
                  <button
                    ...
                    disabled={loading}
                    className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs transition-colors shadow-lg shadow-indigo-500/20"
                  >
          ...
          <div className="max-w-5xl space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="p-6 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border-color)] space-y-4">
                <h3 className="text-sm font-bold mb-2">Configure Backup Action</h3>
              ...
              <div className="flex flex-col rounded-2xl bg-black border border-[var(--border-color)] overflow-hidden h-[380px]">
                <div className="px-4 py-2.5 bg-[var(--bg-secondary)] border-b border-[var(--border-color)] flex items-center justify-between text-xs font-bold text-[var(--text-muted)]">
                  <span className="flex items-center gap-2">
                    <Terminal size={14} /> Live Rclone Terminal Logs
                  </span>
      ...
      {showAddRemoteModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-[var(--bg-secondary)] rounded-2xl p-6 border border-[var(--border-color)] shadow-2xl space-y-4">
            <h3 className="text-sm font-bold">Add Cloud Storage Target</h3>
