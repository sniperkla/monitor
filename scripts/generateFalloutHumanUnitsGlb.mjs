...
const makeMat = ({ roughness = 0.82, metalness = 0.12, ...options }) => makeMaterial({
  roughness,
  metalness,
  ...options,
});
...
const addCyl = (parent, name, radiusTop, radiusBottom, length, radialSegments, material, position, rotation = [0, 0, Math.PI / 2], scale = [1, 1, 1]) => (
  addMesh({ parent, name, geometry: new THREE.CylinderGeometry(radiusTop, radiusBottom, length, Math.min(radialSegments, 6)), material, position, rotation, scale })
);
...
const GEOMETRY = {
  legCapsule: new THREE.CapsuleGeometry(1.34, 8.8, 4, 8),
  torsoCapsule: new THREE.CapsuleGeometry(3.45, 11.2, 4, 8),
  armCapsule: new THREE.CapsuleGeometry(1.12, 9.0, 4, 8),
  handSphere: new THREE.SphereGeometry(1.0, 8, 6),
  shoulderPad: new THREE.SphereGeometry(1.9, 10, 6),
  headSphere: new THREE.SphereGeometry(3.05, 12, 10),
  helmetCrown: new THREE.SphereGeometry(3.25, 10, 8, 0, Math.PI * 2, 0, Math.PI / 2),
  reconHelmetShell: new THREE.SphereGeometry(3.7, 12, 10, 0, Math.PI * 2, 0, Math.PI * 0.62),
  assaultHelmetShell: new THREE.SphereGeometry(3.66, 12, 10, 0, Math.PI * 2, 0, Math.PI * 0.66),
};
...
const SHARED_UNIT_MATERIALS = {
  skin: makeMat({ color: '#b97848', roughness: 0.82, metalness: 0 }),
  belt: makeMat({ color: '#25282d', roughness: 0.76, metalness: 0.12 }),
  canister: makeMat({ color: '#78350f', roughness: 0.66, metalness: 0.26 }),
  pilotFlame: makeMat({ color: '#f97316', emissive: '#f97316', emissiveIntensity: 1.8, transparent: true, opacity: 0.7 }),
};
...
const buildRiflemanWeapon = (parent) => {
  ...
  addCyl(group, 'w_barrel', 0.11, 0.12, 5.8, 6, PALETTE.steelDark, [6.8, 0.12, 0]);
  ...
  addCyl(group, 'w_scope_body', 0.24, 0.26, 2.2, 6, PALETTE.steelDark, [0.4, 0.98, 0]);
  addMesh({ parent: group, name: 'w_scope_lens', geometry: new THREE.CircleGeometry(0.18, 8), material: PALETTE.lens, position: [1.52, 0.98, 0], rotation: [0, Math.PI / 2, 0] });
...
const buildMarksmanWeapon = (parent) => {
  ...
  addCyl(group, 'w_barrel', 0.14, 0.13, 8.2, 6, PALETTE.steelDark, [8.2, 0.12, 0]);
  addCyl(group, 'w_muzzle_brake', 0.22, 0.2, 0.9, 6, PALETTE.steelLight, [12.55, 0.12, 0]);
  ...
  addCyl(group, 'w_scope_tube', 0.32, 0.32, 4.2, 6, PALETTE.steelDark, [0.4, 1.12, 0]);
  addCyl(group, 'w_scope_obj', 0.42, 0.34, 0.9, 6, PALETTE.steelDark, [2.65, 1.12, 0]);
  addMesh({ parent: group, name: 'w_scope_lens', geometry: new THREE.CircleGeometry(0.3, 8), material: PALETTE.lens, position: [3.15, 1.12, 0], rotation: [0, Math.PI / 2, 0] });
...
const buildGunnerWeapon = (parent) => {
  ...
  addCyl(group, 'w_barrel', 0.16, 0.15, 7.2, 6, PALETTE.steelDark, [8.5, 0.34, 0]);
...
const buildRpgWeapon = (parent) => {
  ...
  addCyl(group, 'w_tube_main', 0.58, 0.54, 10.2, 6, makeMat({ color: '#62744d', roughness: 0.78, metalness: 0.16 }), [0.8, 0, 0]);
  addCyl(group, 'w_tube_rear', 0.82, 0.62, 2.1, 6, PALETTE.steelDark, [-4.8, 0, 0]);
  ...
  addCyl(group, 'w_scope_eye', 0.24, 0.26, 0.8, 6, PALETTE.steelDark, [-0.72, 0.78, -0.58], [Math.PI / 2, 0, 0]);
  addMesh({ parent: group, name: 'w_scope_lens', geometry: new THREE.CircleGeometry(0.18, 8), material: PALETTE.lens, position: [-0.72, 0.78, -1.0], rotation: [Math.PI / 2, 0, 0] });
  addCyl(group, 'w_booster_motor', 0.46, 0.48, 3.0, 6, PALETTE.steelMid, [2.8, 0, 0]);
  addCyl(group, 'w_warhead_body', 0.72, 0.48, 2.8, 6, PALETTE.steelMid, [6.1, 0, 0]);
  addMesh({ parent: group, name: 'w_warhead_cone', geometry: new THREE.ConeGeometry(0.2, 1.3, 6), material: PALETTE.hazard, position: [7.75, 0, 0], rotation: [0, 0, Math.PI / 2] });
...
const buildMissileWeapon = (parent) => {
  ...
  addCyl(group, 'w_launch_tube', 0.6, 0.6, 10.1, 6, makeMat({ color: '#5d7851', roughness: 0.76, metalness: 0.16 }), [1.5, 0, 0]);
  addCyl(group, 'w_seeker_dome', 0.52, 0.48, 0.78, 6, PALETTE.steelLight, [6.7, 0, 0]);
  addMesh({ parent: group, name: 'w_seeker_lens', geometry: new THREE.CircleGeometry(0.32, 8), material: PALETTE.lens, position: [7.14, 0, 0], rotation: [0, Math.PI / 2, 0] });
  addCyl(group, 'w_missile_body', 0.42, 0.42, 8.8, 6, PALETTE.steelDark, [1.2, 0, 0]);
  addMesh({ parent: group, name: 'w_missile_cone', geometry: new THREE.ConeGeometry(0.38, 1.5, 6), material: PALETTE.steelMid, position: [5.9, 0, 0], rotation: [0, 0, Math.PI / 2] });
...
const buildEngineerTool = (parent) => {
  ...
  addCyl(group, 'w_wand_body', 0.28, 0.3, 3.4, 6, PALETTE.steelMid, [1.2, 0, 0]);
  addCyl(group, 'w_wand_nozzle', 0.18, 0.28, 1.5, 6, PALETTE.steelDark, [3.1, 0.02, 0]);
  ...
  addMesh({ parent: group, name: 'w_pilot_flame', geometry: new THREE.ConeGeometry(0.09, 0.36, 6), material: SHARED_UNIT_MATERIALS.pilotFlame, position: [4.15, 0.38, 0.18], rotation: [0, 0, Math.PI / 2] });
};
...
const buildSoldier = ({ name, clothColor, armorColor, accentColor, helmetColor, visorColor = '#cbd5e1', visorEmissive = '#000000', visorEmissiveIntensity = 0, helmetType = 'assault', heavy = false, engineer = false, buildWeaponFn }) => {
  ...
  const skinMat = SHARED_UNIT_MATERIALS.skin;
  const beltMat = SHARED_UNIT_MATERIALS.belt;
  ...
  addMesh({ parent: legLeft, name: 'unit_leg_left_mesh', geometry: GEOMETRY.legCapsule, material: clothMat, position: [0, -4.8, 0] });
  ...
  addMesh({ parent: legRight, name: 'unit_leg_right_mesh', geometry: GEOMETRY.legCapsule, material: clothMat, position: [0, -4.8, 0] });
  ...
  const bootLeft = addPivot(legLeft, 'unit_boot_left', [0, -8.0, 0.4]);
  addBox(bootLeft, 'unit_boot_left_mesh', 2.4, 1.6, 4.9, PALETTE.rubber, [0, -1.08, 0.7]);
  addCyl(bootLeft, 'unit_boot_ankle_l', 1.28, 1.36, 1.2, 6, PALETTE.rubber, [0, -0.26, -0.32], [0, 0, 0]);

  const bootRight = addPivot(legRight, 'unit_boot_right', [0, -8.0, 0.4]);
  addBox(bootRight, 'unit_boot_right_mesh', 2.4, 1.6, 4.9, PALETTE.rubber, [0, -1.08, 0.7]);
  addCyl(bootRight, 'unit_boot_ankle_r', 1.28, 1.36, 1.2, 6, PALETTE.rubber, [0, -0.26, -0.32], [0, 0, 0]);

  const torso = addPivot(root, 'unit_torso', [0, 15.4, 0]);
  coreMeshes.push(addMesh({ parent: torso, name: 'unit_torso_mesh', geometry: GEOMETRY.torsoCapsule, material: clothMat, position: [0, 0, 0] }));
  coreMeshes.push(addBox(torso, 'unit_torso_chest_core', 7.6, 6.4, 1.0, armorMat, [0, 1.0, 1.9]));
  coreMeshes.push(addBox(torso, 'unit_torso_chest_trim', 7.42, 6.22, 1.08, armorMat, [0, 1.0, 2.12], [0, 0, 0], [1, 0.96, 0.92]));
  coreMeshes.push(addBox(torso, 'unit_torso_ab_core', 5.4, 3.6, 0.8, accentMat, [0, -2.4, 1.8]));
  coreMeshes.push(addBox(torso, 'unit_torso_ab_trim', 5.22, 3.42, 0.88, accentMat, [0, -2.4, 1.98], [0, 0, 0], [1, 0.96, 0.92]));
  ...
  coreMeshes.push(addBox(vest, 'unit_vest_front', 8.2, 7.8, 1.5, armorMat, [0, 0, 0]));
  coreMeshes.push(addBox(vest, 'unit_vest_back_plate', 7.2, 7.4, 1.2, armorMat, [0, -0.1, -4.2]));
  coreMeshes.push(addBox(vest, 'unit_vest_side_l', 1.2, 5.7, 3.8, armorMat, [-4.4, -0.9, -2.0]));
  coreMeshes.push(addBox(vest, 'unit_vest_side_r', 1.2, 5.7, 3.8, armorMat, [4.4, -0.9, -2.0]));
  for (let index = 0; index < (heavy ? 4 : 3); index += 1) {
    coreMeshes.push(addBox(vest, `unit_vest_pouch_${index}`, 1.7, 2.0, 0.84, accentMat, [(-1.5 + index) * 2.1, -2.8, 0.52]));
  }
  ...
  coreMeshes.push(addBox(pack, 'unit_pack_mesh', engineer ? 6.6 : 5.8, engineer ? 8.8 : 7.2, 4.0, armorMat, [0, 0, 0]));
  coreMeshes.push(addBox(pack, 'unit_pack_top_flap', 5.2, 1.0, 3.4, accentMat, [0, 4.4, 0.2]));
  coreMeshes.push(addBox(pack, 'unit_pack_front_zip', 3.7, 3.2, 0.5, accentMat, [0, -1.1, -2.02]));

  const armLeft = addPivot(root, 'unit_arm_left', [-4.9, 19.1, 0.7]);
  addMesh({ parent: armLeft, name: 'unit_arm_left_mesh', geometry: GEOMETRY.armCapsule, material: clothMat, position: [-0.52, -3.4, 0], rotation: [0.08, 0, 0.24] });
  addBox(armLeft, 'unit_elbow_l', 2.1, 1.26, 1.4, accentMat, [-0.92, -6.1, -0.1]);
  addMesh({ parent: armLeft, name: 'unit_hand_l', geometry: GEOMETRY.handSphere, material: beltMat, position: [-0.9, -8.8, 0.35], scale: [0.96, 1.26, 0.8] });

  const armRight = addPivot(root, 'unit_arm_right', [4.8, 18.9, 1.2]);
  addMesh({ parent: armRight, name: 'unit_arm_right_mesh', geometry: GEOMETRY.armCapsule, material: clothMat, position: [0.56, -3.42, 0.3], rotation: [0.24, 0.04, -0.62] });
  addBox(armRight, 'unit_elbow_r', 2.1, 1.26, 1.4, accentMat, [0.82, -6.08, 0.24]);
  addMesh({ parent: armRight, name: 'unit_hand_r', geometry: GEOMETRY.handSphere, material: beltMat, position: [0.92, -8.82, 0.84], scale: [0.96, 1.26, 0.8] });

  const padLeft = addPivot(root, 'unit_pad_left', [-4.8, 19.0, 0.7]);
  addMesh({ parent: padLeft, name: 'unit_pad_left_mesh', geometry: GEOMETRY.shoulderPad, material: armorMat, position: [0, 0, 0], scale: [1, 0.58, 0.92] });
  const padRight = addPivot(root, 'unit_pad_right', [4.8, 18.7, 1.2]);
  addMesh({ parent: padRight, name: 'unit_pad_right_mesh', geometry: GEOMETRY.shoulderPad, material: armorMat, position: [0, 0, 0], scale: [1, 0.58, 0.92] });

  const head = addPivot(root, 'unit_head', [0, 23.9, 0.28]);
  addMesh({ parent: head, name: 'unit_head_mesh', geometry: GEOMETRY.headSphere, material: skinMat, position: [0, 1.36, 0.1] });

  const neckWrap = addPivot(root, 'unit_neck_wrap', [0, 22.4, 0.3]);
  addCyl(neckWrap, 'unit_neck_wrap_mesh', 1.7, 1.86, 2.0, 6, accentMat, [0, 0, 0], [0, 0, 0]);

  const helmet = addPivot(root, 'unit_helmet', [0, 25.7, 0.18]);
  if (helmetType === 'marksman') {
    addCyl(helmet, 'unit_helmet_shell', 4.6, 4.2, 1.9, 6, helmetMat, [0, 1.3, 0], [0, 0, 0]);
    addMesh({ parent: helmet, name: 'unit_helmet_crown', geometry: GEOMETRY.helmetCrown, material: helmetMat, position: [0, 1.2, 0] });
  } else if (helmetType === 'recon') {
    addMesh({ parent: helmet, name: 'unit_helmet_shell', geometry: GEOMETRY.reconHelmetShell, material: helmetMat, position: [0, 0.45, 0] });
    ...
    addMesh({ parent: helmet, name: 'unit_helmet_shell', geometry: GEOMETRY.assaultHelmetShell, material: helmetMat, position: [0, 0.24, 0], scale: [1, 0.82, 1] });
  ...
  if (engineer) {
    ...
    addCyl(toolCanister, 'unit_tool_canister_mesh', 1.18, 1.22, 5.6, 6, PALETTE.hazard, [0, 0, 0], [0.12, 0, 0.2]);
    addBox(root, 'unit_tool_canister2', 1.9, 4.8, 1.9, SHARED_UNIT_MATERIALS.canister, [0.6, 13.8, -5.3]);
    ...
    addCyl(torch, 'unit_torch_mesh', 0.24, 0.24, 5.2, 6, PALETTE.hazard, [0, 0, 0], [0.3, 0.12, -0.22]);
    addBox(torch, 'unit_torch_handle', 0.5, 1.62, 0.5, PALETTE.rubber, [0, -1.62, 0]);
  }

  mergeMeshGroupByMaterial({ parent: root, name: 'unit_core_merged', meshes: coreMeshes });
  [torso, vest, pack].forEach((groupNode) => root.remove(groupNode));
