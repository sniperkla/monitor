...
- Browser checks: `http://localhost:3030` + a normal Chrome UA (headless is blocked by `src/proxy.js`); `npm test` (**534**) + per-file eslint 0 errors.
...
- Ordinary sites render **in-app**, relay-first, with **NO server-proxy fallback** (`frameFor` → `relay` | `relay-required`). `relayProxyPort` ← `fetchRelayStatus()` ← `/api/relay/token` ← the relay's `webproxy:ready` ack.
- LNA / `upgrade-insecure-requests` / `block-all-mixed-content` do **NOT** gate loopback frames (measured). An **opaque** host (`about:blank`) IS refused (a harness bug).
...
- **The relay's `<base href>` must carry the document's DIRECTORY, not just its origin** — it OVERRIDES the document URL, so an origin-only base 404s relative URLs below the root (a `<video src="clip.mp4">` on `/html/page` → `/clip.mp4` → `NETWORK_NO_SOURCE`). Root-absolute sites are unaffected — why it survived. Non-HTML responses **STREAM**.
...
- WebUI proxy: **tunnel coordinates go in the PATH, never the query** (RFC 3986 drops the base query, so `?connectionId=&port=` 400s every lazy chunk). **Hermes uses Vue Router `createWebHashHistory()`**. `tests/webui-proxy-assets.test.mjs`.
- **The asset path key (`ASSET_KEY`, now `m2`) must stay a STATIC path segment** — a query-string epoch splits Hermes' module graph (the modulepreload href keeps it, the lazy import drops it → two module instances → green splash; that is 4680c2d7 vs 8e4f604d). Bump the key to evict an `immutable`-cached poisoned entry; a static segment survives both resolution rules. `tests/webui-proxy-key.test.mjs`.
