...
- **A relay change ships by TAG**: `relay-v*` → `publish-relay.yml` → npm. Bump `RELAY_VERSION` **and** `packages/local-relay/package.json` TOGETHER — an unbumped fix is invisible to npm. `main` is a stub and the DEFAULT branch: re-run the TAG's run, never a UI dispatch. **`gh` is unauthenticated here — use the keychain git PAT** (`git credential fill`).
